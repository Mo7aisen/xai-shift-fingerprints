...
\title[Validation-Locked Attribution Fingerprints for CXR Shift Monitoring]{Validation-Locked Attribution Fingerprints for Auditable Domain-Shift Monitoring in Chest X-ray Segmentation}
...
\abstract{\textbf{Background:} We present a validation-locked attribution-fingerprinting framework for detecting and characterizing clinically relevant domain shift in chest X-ray segmentation. The framework is designed to make deployment decisions reviewer-traceable by separating discrimination, robustness, and clinical linkage into explicit acceptance gates. \textbf{Methods:} The primary evidence is a 20-seed Phase 1 Extended bidirectional evaluation on JSRT and Shenzhen, with endpoint-specific assessment for predicted-mask and mask-free operating modes. We additionally executed a predeclared hardshift extension in the Shenzhen setting under frozen governance and unchanged decision thresholds. \textbf{Results:} In the primary analysis, the mask-free endpoint showed the strongest reverse-direction operating profile (AUROC 0.9905, 95\% CI 0.9849 to 0.9953; FPR95 0.0389; perturbation degradation 2.03\%; Gate-5 clinical PASS), whereas the predicted-mask endpoint reached high discrimination (AUROC up to 0.9944) but remained too fragile under robustness stress testing. In the hardshift extension, both promoted candidates failed Gate 3 because estimated power remained below the prespecified threshold. \textbf{Conclusions:} These findings support a constrained deployment-oriented use case for the mask-free endpoint, demonstrate the value of 20-seed validation-locked auditing, and define a current boundary of the method in the harder Shenzhen hardshift setting through an explicitly documented negative-result branch.}
...
The hardshift extension yields an equally important negative finding. The method can be reproducibly weak in a harder setting. Both promoted hardshift candidates showed stable seed-level rankings yet poor discriminative power, which indicates a genuine method boundary rather than a noisy or inconclusive result. This negative branch strengthens the manuscript by documenting where the current method stops working under predeclared evaluation.
...
Validation-locked attribution fingerprints provide an interpretable and auditable mechanism for monitoring domain shift in chest X-ray segmentation. In the primary Phase 1 Extended bidirectional evaluation, the mask-free endpoint offers the most favorable balance of discrimination, robustness, and clinical linkage. The attempted hardshift extension did not pass predeclared validation and is therefore retained as an honest negative result rather than a positive deployment claim. Taken together, these findings support a narrow, defensible conclusion: attribution fingerprints can support deployment-focused shift monitoring when endpoint-specific gate criteria are enforced, but the current method does not yet support a successful hardshift claim in the Shenzhen setting.
...
JSRT and Shenzhen are publicly available from their official sources. Derived audit artifacts, governance summaries, and reviewer-traceable freeze manifests that support the negative-result hardshift branch are provided in Additional files 1 to 3. A repository-orientation note for reviewers is provided in Additional file 4.

\subsection*{Code availability}
All source code, configurations, analysis scripts, governance manifests, and reproducibility assets are available in the public project repository: \url{https://github.com/Mo7aisen/xai-shift-fingerprints}. The submission-aligned repository state is the public \texttt{main} branch as prepared for submission on 2026-02-28. The repository contains the exact validation-locked 20-seed primary pipeline used for the main study and the negative-result hardshift extension documented here. A DOI-backed archive snapshot will be minted upon acceptance.
...
\textbf{Additional file 1.} Hardshift final verification report (\url{HARDSHIFT_PROJECT_FINAL_VERIFICATION_2026-02-28.md}). This file summarizes the closure state of the hardshift branch and the preserved reviewer-traceable evidence set.

\textbf{Additional file 2.} Hardshift final freeze manifest (\url{hardshift_project_final_freeze_2026-02-28.sha256}). This file contains checksums for the preserved project artifacts used in the submission package.

\textbf{Additional file 3.} Hardshift submission bundle index (\url{hardshift_project_submission_bundle_2026-02-28.md}). This file provides the reviewer-facing bundle index for the hardshift governance and audit trail.

\textbf{Additional file 4.} Repository access and reproducibility note (\url{REPOSITORY_ACCESS_AND_REPRODUCIBILITY.md}). This file describes the public code location and the expected reproducibility layout for reviewers.
