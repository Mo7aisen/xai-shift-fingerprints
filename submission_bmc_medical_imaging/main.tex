...
\title[Validation-Locked Attribution Fingerprints for CXR Shift Monitoring]{Validation-Locked Attribution Fingerprints for Auditable Shift Monitoring in Chest X-ray Segmentation}
...
\abstract{\textbf{Background:} We present a validation-locked attribution-fingerprinting framework for detecting clinically relevant shift in chest X-ray segmentation. The framework is designed to support reviewer-traceable deployment assessment by separating discrimination, robustness, and clinical linkage into prespecified acceptance gates. \textbf{Methods:} We performed a 20-seed Phase 1 Extended bidirectional evaluation on JSRT and Shenzhen, with endpoint-specific analysis for predicted-mask and mask-free operating modes. We additionally ran a predeclared hardshift extension in the Shenzhen setting under frozen governance and unchanged decision thresholds. \textbf{Results:} In the primary analysis, the mask-free endpoint showed the strongest reverse-direction operating profile (AUROC 0.9905, 95\% CI 0.9849 to 0.9953; FPR95 0.0389; perturbation degradation 2.03\%; Gate-5 clinical PASS), whereas the predicted-mask endpoint reached high discrimination (AUROC up to 0.9944) but remained too fragile under robustness stress testing. In the hardshift extension, both promoted candidates failed Gate 3 because estimated power remained below the prespecified threshold of 0.80. \textbf{Conclusions:} These findings support a narrow deployment-oriented use case for the mask-free endpoint, demonstrate the value of 20-seed validation-locked auditing, and define a current boundary of the method through an explicitly documented negative-result hardshift branch.}
